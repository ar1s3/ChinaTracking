    ...
    public function runProject($project_token, $options = array())
    {
        ...
        if ($this->isResponseValid($response)) {
            ...
            if (isset($run_object->project_token) && isset($run_object->run_token)) {
                self::$logger->info('Project run successfully on parsehub with values: ', [
                    'context' => array(
                        'start_url' => $start_url,
                        'keywords' => $keywords,
                ...
                self::$logger->info('Project already running on parsehub with same values: ', [
                    'context' => array(
                        'start_url' => $start_url,
                        'keywords' => $keywords,
                        'send_email' => $send_email,
                ...
                self::$logger->info('Unable to start project on parsehub: ', [
                    'context' => array(
                        'start_url' => $start_url,
                        'keywords' => $keywords,
